...
import httplib
import urllib
from utils import build_interfaces_by_method
...
class Resource(object):
    ...
    def _request(self, endpoint=None, **kwargs):
        ...
        for k in resource.get('required', []):
            if k not in (x.split(':')[0] for x in iterkeys(kwargs)):
        ...
        if not method:
            raise InterfaceNotDefined(
                'Interface is not defined, you must pass' +
                ' ``method`` (HTTP Method).')
        ...
        if response.status != 200:
            raise ERROR_MAP.get(data['code'],
                                APIError)(data['code'], data['response'])
...
class DisqusAPI(Resource):
    ...
    def key(self):
        warnings.warn(
            "'key' is deprecated in favor of directly accessing the " +
    ...
    def setSecretKey(self, key):
        warnings.warn(
            "'setSecretKey' is deprecated in favor of directly setting" +
    ...
    def setKey(self, key):
        warnings.warn(
            "'setKey' is deprecated in favor of directly setting the" +
            " 'secret_key' attribute",
            DeprecationWarning)
        self.secret_key = key
    ...
    def setPublicKey(self, key):
        warnings.warn(
            "'setPublicKey' is deprecated in favor of directly setting" +
    ...
    def setFormat(self, format):
        warnings.warn(
            "'setFormat' is deprecated in favor of directly setting the" +
            " 'format' attribute",
            DeprecationWarning)
        self.format = format
    ...
    def setVersion(self, version):
        warnings.warn(
            "'setVersion' is deprecated in favor of directly setting the" +
            " 'version' attribute",
            DeprecationWarning)
        self.version = version
    ...
    def setTimeout(self, timeout):
        warnings.warn(
            "'setTimeout' is deprecated in favor of directly setting the" +
            " 'timeout' attribute",
            DeprecationWarning)
